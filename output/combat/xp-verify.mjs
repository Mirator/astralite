...
const out='output/combat/xp-verify'; fs.mkdirSync(out,{recursive:true});
...
let held=[];const log=[];
for(let i=0;i<400;i++){
 s=await state(); assert.equal(s.experience.total,(5-s.remaining)*25,'exactly one XP reward per kill'); assert.equal(Number(await page.getByRole('progressbar',{name:'Encounter experience'}).getAttribute('value')),s.experience.total,'HUD matches XP'); if(s.mode!=='playing')break;
...
await shot('result');s=await state();fs.writeFileSync(`${out}/combat-log.json`,JSON.stringify(log,null,2));assert.equal(s.mode,'won','first encounter clears'); assert.equal(s.experience.total,125,'all five rewards recorded'); await step(2000); assert.equal((await state()).experience.total,125,'dead enemies never award XP again');
await page.getByRole('button',{name:'TRY AGAIN'}).click();await page.waitForFunction(()=>window.render_game_to_text && JSON.parse(window.render_game_to_text()).mode==='playing');await step(0);assert.equal((await state()).health,100,'restart restores health'); assert.equal((await state()).experience.total,0,'new run resets XP');
await step(20000);await shot('loss');assert.equal((await state()).mode,'lost','loss transition');
assert.deepEqual(errors,[]); fs.writeFileSync(`${out}/report.json`,JSON.stringify({passed:true,winHealth:s.health,experience:s.experience,errors},null,2));await browser.close();
