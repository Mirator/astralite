...
fs.writeFileSync('output/floor/run.json',JSON.stringify(result,null,2));await page.screenshot({path:'output/floor/result.png'});assert.equal(result.s.mode,'won');assert.equal(result.s.experience.total,550);assert.equal(result.s.floor.visited.length,12);
...
await page.setViewportSize({width:390,height:844});await page.screenshot({path:'output/floor/mobile.png'});assert.deepEqual(errors,[]);console.log(JSON.stringify({minimumTiles:minimum,seeds:500,run:result.s.mode,health:result.s.health,steps:result.i,errors}));await browser.close();
