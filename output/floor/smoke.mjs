...
fs.writeFileSync('output/floor/short-run.json',JSON.stringify(result,null,2));await page.screenshot({path:'output/floor/corridor.png'});assert.equal(result.s.mode,'playing');assert(result.s.floor.visited.length>1);assert.equal(result.s.experience.total,(22-result.s.remaining)*25);
const seed=result.s.floor.seed;await page.reload();await page.waitForFunction(seed=>window.render_game_to_text&&JSON.parse(window.render_game_to_text()).floor.seed!==seed,seed);await page.evaluate(()=>window.advanceTime(0));assert.equal((await page.evaluate(()=>JSON.parse(window.render_game_to_text()))).experience.total,0);
await page.setViewportSize({width:390,height:844});await page.evaluate(()=>window.advanceTime(0));await page.screenshot({path:'output/floor/mobile.png'});assert.deepEqual(errors,[]);console.log(JSON.stringify({minimumTiles:minimum,seeds:500,run:result.s.mode,health:result.s.health,steps:result.i,errors}));await browser.close();
