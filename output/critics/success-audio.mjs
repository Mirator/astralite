...
await page.addInitScript(()=>{const Native=window.AudioContext;window.AudioContext=class extends Native {constructor(...args){super(...args);window.testAudio=this;}};});await page.goto('http://localhost:3000');await page.waitForFunction(()=>!!window.render_game_to_text);
...
for(let level=1;level<=1;level++){
  ...
  s=await state();assert.equal(await page.evaluate(()=>window.testAudio.state),'running','victory audio remains audible');assert.equal(s.mode,'complete');assert.equal(s.floor.level,level);assert.equal(s.objective.stairClear,true);
