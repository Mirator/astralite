...
const browser=await chromium.launch({headless:true,args:['--use-gl=angle','--use-angle=swiftshader']});const page=await browser.newPage({viewport:{width:1100,height:760}});const errors=[];page.on('pageerror',e=>errors.push(String(e)));
await page.goto('http://localhost:3000');await page.waitForFunction(()=>!!window.render_game_to_text,{timeout:60000});
await page.evaluate(()=>window.advanceTime(0));await page.screenshot({path:'output/polish/intro.png'});await page.getByRole('button',{name:'ENTER THE KEEP',exact:false}).click();await page.evaluate(()=>window.advanceTime(0));
...
await page.exposeFunction('captureCombat',async()=>page.screenshot({path:'output/polish/combat.png'}));
const result=await page.evaluate(async ({tiles})=>{
 ...
 let lastRemaining=22, captured=false;
 for(let i=0;i<700;i++){
  const s=state();if(!captured&&s.enemies.some(e=>e.windup>0)){captured=true;document.querySelector('canvas').getContext('webgl2').finish();await window.captureCombat();}if(s.mode!=='playing'){input([]);return {s,i,snapshots};}
  ...
  if(i%20===0){document.querySelector('canvas').getContext('webgl2').finish();await new Promise(r=>setTimeout(r,0));}
 }
 input([]);return {s:state(),snapshots};
},{tiles:f.tiles});
fs.writeFileSync('output/polish/loss.json',JSON.stringify(result,null,2));await page.screenshot({path:'output/polish/loss.png'});assert.equal(result.s.mode,'lost');assert.equal(result.s.experience.total,0);assert.equal(result.s.health,0);
...
await page.getByRole('button',{name:'ENTER THE KEEP',exact:false}).click();await page.evaluate(()=>window.advanceTime(0));await page.setViewportSize({width:390,height:844});await page.evaluate(()=>window.advanceTime(0));
